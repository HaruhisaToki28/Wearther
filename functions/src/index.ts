...
// リクエストの型定義
interface FashionAdviceRequest {
  weather: {
    condition: string;      // 天気（晴れ、くもり、雨など）
    temperature: number;    // 最高気温
    minTemperature: number; // 最低気温
    precipitationChance: number; // 降水確率
    location: string;       // 地域名
  };
  userPreferences?: {
    gender?: string;        // 性別
    temperatureTolerance?: string; // 寒暖耐性（寒がり、普通、暑がり）
  };
}

// レスポンスの型定義
interface FashionAdviceResponse {
  title: string;       // 1行以内のタイトル（具体的なアイテム）
  description: string; // 2行以内のキャプション（アドバイス）
}
 ...
 */
export const generateFashionAdvice = onCall(
  {
  ...
  async (request): Promise<FashionAdviceResponse> => {
    // リクエストデータの検証
    const data = request.data as FashionAdviceRequest;
    
    if (!data.weather) {
      throw new HttpsError("invalid-argument", "天気データが必要です");
    }
    ...
    try {
      ...
      const advice = JSON.parse(jsonMatch[0]) as FashionAdviceResponse;
...
function buildPrompt(
  weather: FashionAdviceRequest["weather"],
  userPreferences?: FashionAdviceRequest["userPreferences"]
): string {
